...
2. Use a `Mutex<()>` or `AtomicBool` CAS spin to make it safe under concurrent access
3. Change to `&mut self` to enforce single access at the type level
...
2. **H1** — Wire up WonderEngine LLM call (core feature gap, 1-2 hours)
3. **M4** — Add automatic budget reset (consciousness daemon is broken without this)
4. **M1** — Fix UTF-8 truncation panic (will crash on emoji/unicode)
5. **M2** — Fix token estimation (budget accuracy)
6. **M5** — Store base budget values as fields (prevents future drift)
7. **M3** — Document or fix semaphore TOCTOU
8. **L1-L8** — Address in next cleanup pass
...
2. **"ConsciousnessBudget — Token/cost budget with quiet hours (10PM–7AM) and entropy-based scaling"** — Quiet hours work, entropy scaling works, but the budget counters never reset (M4), so the budget effectively becomes permanent after first exhaustion.
