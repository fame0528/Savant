    ...
    if let Ok(_rt) = tokio::runtime::Handle::try_current() {
        if let Ok(guard) = ENGINE.try_read() {
            if let Some(engine) = guard.as_ref() {
                ...
                return engine.compact(&output);
            }
        }
    }
    if let Ok(rt) = tokio::runtime::Handle::try_current() {
        let result = rt.block_on(async {
            compact_output(tool_name, &parse_argv_from_args(args), exit_code, raw_output, working_dir).await
        });
        return result;
    }
...
fn parse_argv_from_args(args: &str) -> Vec<String> {
    if let Ok(val) = serde_json::from_str::<serde_json::Value>(args) {
        ...
        if let Some(obj) = val.as_object() {
            ...
            return obj.values().filter_map(|v| v.as_str().map(String::from)).collect();
        }
    }
    if args.is_empty() { vec![] } else { vec![args.to_string()] }
}
