...
impl DelegationEngine {
    ...
    pub fn with_rate_limiter(self: Arc<Self>, limiter: Arc<crate::rate_limiter::RateLimiter>) -> Arc<Self> {
    ...
    pub fn publish_event(&self, event: SubAgentEvent) {
        match &event {
            ...
            SubAgentEvent::Completed { id, profile, success, duration_ms, .. } => {
                tracing::info!(
                    subagent_id = %id,
                    profile = %profile,
                    success = success,
            ...
            SubAgentEvent::Failed { id, profile, error, .. } => {
                tracing::warn!(
                    subagent_id = %id,
                    profile = %profile,
                    error = %error,
...
mod tests {
    ...
    async fn test_route_keyword_matching() {
        ...
        assert_eq!(engine.route("fix the cargo build error", "").await, "coding");
        assert_eq!(engine.route("write the readme documentation", "").await, "documentation");
        assert_eq!(engine.route("search for all uses of this function", "").await, "research");
        assert_eq!(engine.route("write tests for this module", "").await, "testing");
