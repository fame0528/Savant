...
impl LoopDetector {
    ...
    pub fn record_call(&mut self, tool_name: &str, params_hash: &str, success: bool) -> LoopDetection {
        self.total_calls += 1;
...
mod tests {
    ...
    fn test_identical_loop_detection() {
        ...
        assert_eq!(detector.record_call("read", "file.rs", true), LoopDetection::Clear);
        assert_eq!(detector.record_call("read", "file.rs", true), LoopDetection::Clear);
        assert_eq!(detector.record_call("read", "file.rs", true), LoopDetection::Clear);
        let result = detector.record_call("read", "file.rs", true);
        assert!(matches!(result, LoopDetection::IdenticalLoop { count: 4, .. }));
    }
    ...
    fn test_failing_loop_detection() {
        ...
        assert_eq!(detector.record_call("write", "a.rs", false), LoopDetection::Clear);
        assert_eq!(detector.record_call("write", "b.rs", false), LoopDetection::Clear);
    ...
    fn test_failure_streak_resets_on_success() {
        ...
        assert_eq!(detector.record_call("write", "a.rs", false), LoopDetection::Clear);
        assert_eq!(detector.record_call("write", "b.rs", false), LoopDetection::Clear);
        assert_eq!(detector.record_call("read", "c.rs", true), LoopDetection::Clear); // reset
        assert_eq!(detector.record_call("write", "d.rs", false), LoopDetection::Clear); // streak = 1
    }
    ...
    fn test_max_calls_exceeded() {
        ...
        assert!(matches!(result, LoopDetection::MaxCallsExceeded { total: 4, max: 3 }));
    }
}
