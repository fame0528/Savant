...
/// 9Router provider — routes through 9router's OpenAI-compatible API.
/// 9router handles fallback across 40+ providers, RTK token savings,
/// and format translation. See https://github.com/decolua/9router
...
impl NineRouterProvider {
    /// Builds the full chat completions URL for the 9router instance.
...
#[async_trait]
impl LlmProvider for NineRouterProvider {
    async fn stream_completion(
        &self,
        messages: Vec<ChatMessage>,
        tools: Vec<serde_json::Value>,
    ) -> Result<Pin<Box<dyn Stream<Item = Result<ChatChunk, SavantError>> + Send>>, SavantError>
    {
        let response = self
            ...
            .json(&json!({
                ...
                "frequency_penalty": self.llm_params.as_ref().map(|p| p.frequency_penalty).unwrap_or(0.0),
                "presence_penalty": self.llm_params.as_ref().map(|p| p.presence_penalty).unwrap_or(0.0),
                "max_tokens": self.max_completion_tokens.unwrap_or_else(|| {
                    self.context_window
                        .map(|cw| ((cw as f64 * 0.85) as u32).min(16384))
                        .unwrap_or(4096)
                }),
            ...
            .send()
            .await
            .map_err(|e| classify_http_error(e, "NineRouter"))?;
