...
use crate::react::{ReActLoop, AgentEvent};
...
impl HeartbeatPulse {
    pub fn new(agent: AgentConfig, nexus: Arc<NexusBridge>, storage: Arc<Storage>) -> Self {
        ...
        Self { agent, heartbeat_file, nexus, storage }
    }

    /// Starts the heartbeat loop for this agent.
    pub async fn start(self, mut react_loop: ReActLoop) {
        // Subscribe to chat messages
        let mut chat_rx = self.nexus.subscribe("chat.message").await;
        
        info!("[{}] Heartbeat loop active (Non-blocking mode)", self.agent.agent_name);
            ...
            tokio::select! {
                ...
                Ok(chat_event) = chat_rx.recv() => {
                    if chat_event.event_type == "chat.message" {
                        if let Err(e) = self.handle_chat_message(chat_event.payload, &mut react_loop).await {
                ...
                _ = tokio::time::sleep(Duration::from_secs(self.agent.heartbeat_interval)) => {
                    if let Err(e) = self.pulse(&mut react_loop).await {
    ...
    async fn handle_chat_message(&self, chat_event: String, react_loop: &mut ReActLoop) -> Result<(), SavantError> {
        let chat_message: Result<savant_core::types::ChatMessage, _> = serde_json::from_str(&chat_event);
        ...
        match chat_message {
            Ok(message) => {
                ...
                info!("[{}] Received chat message targeting {:?}: {}", self.agent.agent_name, message.recipient, message.content);
                
                // Process the message through ReAct loop
                let mut full_response = String::new();
                
                {
                    let mut stream = react_loop.run(message.content);
                    while let Some(event) = stream.next().await {
                        match event {
                            ...
                            Ok(AgentEvent::Action { name, args }) => {
                                info!("[{}] Chat Action: {}[{}]", self.agent.agent_name, name, args);
                            }
                            Ok(AgentEvent::FinalAnswer(a)) => {
                                // FinalAnswer might be different from Thought collection
                            ...
                            Ok(_) => {}
                            Err(e) => {
                                tracing::error!("[{}] ReAct Loop Error: {}", self.agent.agent_name, e);
                                return Err(SavantError::Unknown(e));
                            }
                        }
                    }
                }
                ...
                self.nexus.publish("chat.response", &response_payload).await
                    .map_err(|e| SavantError::Unknown(e.to_string()))?;
                
                info!("[{}] Chat response sent", self.agent.agent_name);
            }
            Err(e) => {
                info!("[{}] Invalid chat message format: {}", self.agent.agent_name, e);
            }
        }
    ...
    async fn pulse(&self, react_loop: &mut ReActLoop) -> Result<(), SavantError> {
        ...
        {
            let mut stream = react_loop.run(prompt);
            while let Some(event) = stream.next().await {
                match event {
                    Ok(AgentEvent::Thought(t)) => {
                        full_response.push_str(&t);
                    ...
                    Ok(AgentEvent::Action { name, args }) => {
                        info!("[{}] Proactive Action: {}[{}]", self.agent.agent_name, name, args);
                        action_taken = true;
                    }
                    Ok(AgentEvent::FinalAnswer(a)) => {
                        full_response = a;
                    ...
                    Ok(_) => {}
                    Err(e) => return Err(SavantError::Unknown(e)),
                }
            }
        }

        if !action_taken && full_response.trim() == "HEARTBEAT_OK" {
            info!("[{}] Heartbeat nominal. Silence maintained.", self.agent.agent_name);
            return Ok(());
        }

        info!("[{}] HEARTBEAT INITIATIVE: Agent is speaking up.", self.agent.agent_name);
        
        if let Err(e) = self.storage.append_chat(&self.agent.agent_id, &savant_core::types::ChatMessage {
            role: savant_core::types::ChatRole::Assistant,
            ...
            parsing::log_agent_error(&self.agent.agent_name, "Failed to persist heartbeat to WAL", e);
        }
        ...
        if let Err(e) = react_loop.consolidate_memory().await {
            parsing::log_agent_error(&self.agent.agent_name, "Memory consolidation error", e);
        }
