...
mod tests {
    ...
    impl Tool for MockTool {
        ...
        async fn execute(&self, _args: serde_json::Value) -> Result<String, savant_core::error::SavantError> {
            Ok("ok".to_string())
        }
    }
    ...
    fn mock_tools() -> Vec<Arc<dyn Tool>> {
        vec![
            Arc::new(MockTool { name: "read".to_string() }),
            Arc::new(MockTool { name: "write".to_string() }),
            Arc::new(MockTool { name: "cargo".to_string() }),
            Arc::new(MockTool { name: "npm".to_string() }),
        ]
    }
