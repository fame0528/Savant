...
async fn test_full_delegation_pipeline() {
    ...
    // Load profile
    let mut coding = SubAgentProfile::default();
    coding.name = "coding".to_string();
    coding.max_iterations = 3;
    engine.add_profile(coding).await;

    // Route a coding task
    let routed = engine.route("fix the cargo build error in main.rs", "").await;
    ...
    let handle = engine
        .delegate("coding", "Fix the bug".to_string(), "Context here".to_string(), hooks)
        .await
...
async fn test_delegation_with_rejecting_hook() {
    ...
    let mut general = SubAgentProfile::default();
    general.name = "general".to_string();
    general.max_iterations = 1;
    ...
    assert!(err.contains("rejected by on_start hook"), "Error was: {}", err);
}
