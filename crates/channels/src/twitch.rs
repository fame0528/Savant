    ...
    pub fn spawn(self) -> tokio::task::JoinHandle<()> {
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                // Parse PRIVMSG
                if let Some(msg_part) = line.strip_prefix(":").and_then(|s| s.split_once("PRIVMSG"))
                {
                    if let Some((prefix, content)) = msg_part.1.split_once(" :") {
                        let sender = prefix.split('!').next().unwrap_or("unknown");
                        let text = content.trim();
                        if !text.is_empty() {
                            let sid = savant_core::session::SessionMapper::map("twitch", &channel);
