...
mod tests {
    ...
    fn test_redact_openai_api_key() {
        let input = "Using key sk-abc123def456ghi789jkl012mno345pqr678stu901vwx234";
    ...
    fn test_redact_bearer_token() {
        ...
        let input = "Authorization: Bearer aBcDeFgHiJkLmNoPqRsTuVwXyZ0123456789012";
        let result = scan_and_redact(input);
        assert!(result.redaction_count > 0);
    ...
    fn test_redact_generic_password() {
        let input = r#"password = "SuperSecretPassword123456""#;
        let result = scan_and_redact(input);
        assert!(result.redaction_count > 0);
    ...
    fn test_redact_private_tag() {
        let input = "Normal text <private>secret content here</private> more text";
        let result = scan_and_redact(input);
        assert!(result.redaction_count > 0);
    ...
    fn test_multiple_secrets_in_one_input() {
        let input = "AWS key AKIAIOSFODNN7EXAMPLE and GitHub ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnop";
        let result = scan_and_redact(input);
    ...
    #[test]
    fn test_slack_token() {
        let input = "Slack bot token: xoxb-1234567890-1234567890123-ABCDEFGHijklmnop";
        let result = scan_and_redact(input);
        assert!(result.redaction_count > 0);
    ...
    #[test]
    fn test_stripe_key() {
        let input = "Stripe key: sk_live_abc123def456ghi789jkl0";
        let result = scan_and_redact(input);
        assert!(result.redaction_count > 0);
