...
1. **No Agent Capability Advertisement**
...
1. **No Inter-Agent Result Channel**
...
1. **No Task Priority or Inter-Agent Scheduling**
...
1. **No Task Timeout or Cancellation Propagation**
...
1. **No Agent Health Monitoring Between Agents**
...
1. **Text-Based LLM Commands Are Fragile**
...
1. **Detailed design for a typed task delegation protocol** — structs, state machine, lifecycle, result routing
1. **Agent Card / capability advertisement design** — what to include, how to match, how to update dynamically
1. **Memory-aware context passing design** — how to bundle and transfer relevant memory between agents
1. **Inter-agent messaging layer design** — typed messages over shared memory, integrated with existing blackboard
1. **Specific recommendations from A2A implementations** — what to adopt, what to skip, what to improve
1. **Community top-wanted features** — the 10 most requested A2A features and which ones Savant should implement
1. **Code structure recommendations** — which new modules/files to create, which existing files to modify
1. **Integration points** — exactly where in Savant's existing codebase each feature should plug in (file paths, function names)
1. **Testing strategy** — how to test inter-agent communication without flaky integration tests
